...
class LicitaMaisBrasilMonitor {
     ...
     */
    static isLMBLink(link) {
        if (!link)
            return false;
        return link.toLowerCase().includes(exports.LICITA_MAIS_BRASIL_PLATFORM.domain);
    }
    /**
    ...
    static detectPlatform(link) {
        if (!link)
            return null;
        if (link.toLowerCase().includes(exports.LICITA_MAIS_BRASIL_PLATFORM.domain))
            return exports.LICITA_MAIS_BRASIL_PLATFORM;
        return null;
    }
    /**
     * Extrai a URL da Licita Mais Brasil de um campo de link (pode ser multi-link).
     */
    static extractLMBUrl(linkField) {
        if (!linkField)
            return null;
        const parts = linkField.split(',').map(s => s.trim());
        return parts.find(p => p.toLowerCase().includes(exports.LICITA_MAIS_BRASIL_PLATFORM.domain)) || null;
    }
    /**
     ...
     */
    static async fetchChatChannelId(batchId, token) {
        try {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), this.TIMEOUT_MS);
