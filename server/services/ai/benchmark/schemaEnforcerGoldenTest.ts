...
interface TestCase {
    ...
    description: string;
    input: any;
    assertions: ((output: any) => { pass: boolean; detail: string })[];
...
tests.push({
    ...
    assertions: [
        (o) => {
            ...
            const first3 = rft.slice(0, 3).map((r: any) => r.title.toLowerCase());
...
tests.push({
    ...
    assertions: [
        ...
        (o) => {
            const rft = o.requirements.regularidade_fiscal_trabalhista;
            const titles = rft.map((r: any) => r.title.toLowerCase()).join(' | ');
...
tests.push({
    ...
    assertions: [
        ...
        (o) => {
            const qef = o.requirements.qualificacao_economico_financeira;
            const titles = qef.map((r: any) => r.title.toLowerCase()).join(' | ');
...
tests.push({
    ...
    assertions: [
        ...
        (o) => {
            const pc = o.requirements.proposta_comercial;
            const titles = pc.map((r: any) => r.title.toLowerCase());
            const hasGeneric = titles.some((t: string) => /rasura|emenda|irrevogável|vícios|inexequível|preencher.*campo/i.test(t));
...
tests.push({
    ...
    assertions: [
        (o) => {
            ...
            const hasConsorcio = hj.some((r: any) => /consórcio/i.test(`${r.title} ${r.description}`));
...
tests.push({
    ...
    assertions: [
        (o) => {
            const rft = o.requirements.regularidade_fiscal_trabalhista;
            const rftHasReserva = rft.some((r: any) => /reserva.*cargos/i.test(`${r.title} ${r.description}`));
        ...
        (o) => {
            ...
            const dcHasReserva = dc.some((r: any) => /reserva.*cargos/i.test(`${r.title} ${r.description}`));
...
tests.push({
    ...
    assertions: [
        (o) => {
            const dc = o.requirements.documentos_complementares;
            const dcHasMinor = dc.some((r: any) => /n[ãa]o emprego.*menor/i.test(r.title));
...
tests.push({
    ...
    assertions: [
        ...
        (o) => {
            const hj = o.requirements.habilitacao_juridica;
            const hasAto = hj.some((r: any) => /ato constitutivo|contrato social/i.test(r.title));
