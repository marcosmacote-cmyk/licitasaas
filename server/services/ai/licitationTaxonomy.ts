...
export type RequirementCategoryKey =
...
export interface TaxonomyDistinction {
  pair: [string, string];
  rule: string;
  indicators: { first: string[]; second: string[] };
}

// ── Categorias ──

export const REQUIREMENT_CATEGORIES: TaxonomyCategory[] = [
  {
...
export const CRITICAL_DISTINCTIONS: TaxonomyDistinction[] = [
  {
...
// ── Tabela de tipos de objeto com reforços de atenção ──

export interface ObjectTypeProfile {
  key: ObjectType;
  label: string;
  reinforcementAreas: string[];
  criticalCategories: RequirementCategoryKey[];
  typicalRisks: string[];
}

export const OBJECT_TYPE_PROFILES: ObjectTypeProfile[] = [
  {
    key: 'obra_engenharia',
 ...
 */
export function generateTaxonomyPromptBlock(): string {
  ...
  for (const d of CRITICAL_DISTINCTIONS) {
 ...
 */
export function generateObjectTypeReinforcement(objectType: ObjectType): string {
  const profile = OBJECT_TYPE_PROFILES.find(p => p.key === objectType);
  if (!profile) return '';
