...
const declarationTypes_1 = require("../services/ai/declaration/declarationTypes");
const declarationParser_1 = require("../services/ai/declaration/declarationParser");
const declarationValidator_1 = require("../services/ai/declaration/declarationValidator");
const declarationRepair_1 = require("../services/ai/declaration/declarationRepair");
// Create Gemini AI instance for this module
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const genAI = GEMINI_API_KEY ? new genai_1.GoogleGenAI({ apiKey: GEMINI_API_KEY }) : null;
// ═══════════════════════════════════════════════
// ROTA PRINCIPAL — 12 STEPS
// ═══════════════════════════════════════════════
router.post('/generate-declaration', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[Declaration v5] Step 1: "${declarationType}" (${issuerType || 'company'}) style=${style} BID:${biddingProcessId}`);
        ...
        logger_1.logger.info(`[Declaration v5] Step 4: Family → ${family}`);
        const facts = {
        ...
        logger_1.logger.info(`[Declaration v5] Step 3: Facts → org="${orgaoName}" div=${hasDivergence} rep="${representanteName}"`);
...
${company.technicalQualification || 'Nenhum profissional técnico cadastrado.'}`;
        ...
        // ── Step 7: Chamar IA ──
        logger_1.logger.info(`[Declaration v5] Step 7: Calling Gemini (attempt 1)...`);
        ...
        const parsed = (0, declarationParser_1.parseAndSanitize)(rawResponse);
        ...
        const titleResult = (0, declarationValidator_1.validateAndFixTitle)(finalTitle, declarationType);
        if (titleResult.fixed) {
            logger_1.logger.info(`[Declaration v8] Title fixed: "${finalTitle}" → "${titleResult.title}"`);
            finalTitle = titleResult.title;
        }
        // ── Step 9: Validação pós-geração ──
        logger_1.logger.info(`[Declaration v8] Step 9: Validating...`);
        let issues = (0, declarationValidator_1.validateDeclaration)(finalText, facts);
        ...
        if ((0, declarationValidator_1.hasCriticalIssues)(issues)) {
            logger_1.logger.info(`[Declaration v5] Step 10: ${issues.filter(i => i.severity === 'critical').length} critical issues. Repair via IA...`);
            attempts = 2;
            const aiCallFn = (0, declarationRepair_1.createGeminiRepairFn)(genAI.models, gemini_service_1.callGeminiWithRetry, 'gemini-2.5-flash', { tenantId: req.user.tenantId, operation: 'repair_declaration', metadata: { docType: 'declaration' } });
            const repair = await (0, declarationRepair_1.repairDeclaration)(finalText, finalTitle, issues, facts, declarationValidator_1.validateDeclaration, aiCallFn);
            if (repair.improved) {
        ...
        const qualityReport = (0, declarationValidator_1.calculateQualityReport)(issues, corrections, family, attempts);
        logger_1.logger.info(`[Declaration v5] ${(0, declarationValidator_1.summarizeReport)(qualityReport)}`);
        ...
        logger_1.logger.error("[Declaration v5] Fatal error:", error);
        (0, errorHandler_1.handleApiError)(res, error, 'generate-declaration');
    }
});
...
function buildDeclarationPrompt(facts, family, familyContext, editalContext, issuerBlock, customPrompt, isTechnical, style = 'objetiva', editalClause) {
    ...
    const semanticMatch = declarationTypes_1.DECLARATION_SEMANTIC_MAP.find(m => m.keywords.some(kw => declLower.includes(kw.toLowerCase())));
...
2. EXTENSÃO (${(() => { const c = declarationTypes_1.FAMILY_LENGTH_CONSTRAINTS[family]; return `${c.minParagraphs} a ${c.maxParagraphs} parágrafos — ${c.styleHint}`; })()}):
...
11. ANTI-GENERICISMO: EVITE frases ornamentais como: ${declarationTypes_1.ANTI_GENERIC_PHRASES.slice(0, 3).map(p => `"${p}"`).join(', ')}. Prefira linguagem seca e assertiva.
