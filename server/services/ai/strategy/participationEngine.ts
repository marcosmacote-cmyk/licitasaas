...
export function matchCompanyToEdital(companyId: string, schemaV2: any, processId: string): CompanyEditalMatchResult {
    ...
    if (technicalFit.highRiskGaps.length >= 2) hardBlocks.push(`${technicalFit.highRiskGaps.length} lacunas técnicas críticas`);
    if (!technicalFit.hasSufficientProfessionals && schemaV2?.technical_analysis?.exige_responsavel_tecnico) hardBlocks.push('RT ativo obrigatório não encontrado');
    if (documentaryFit.expired.length >= 3) hardBlocks.push(`${documentaryFit.expired.length} documentos vencidos`);
    if (economicFinancialFit.missing.length >= 2) hardBlocks.push(`${economicFinancialFit.missing.length} pendências econômico-financeiras`);
    if (documentaryFit.missing.length >= 5) hardBlocks.push(`${documentaryFit.missing.length} documentos faltantes — risco alto de inabilitação`);
...
export function calculateParticipationScore(matchResult: CompanyEditalMatchResult, schemaV2: any): ParticipationAssessment {
    ...
    const overallScore = Math.round(
        dims.documentaryReadiness * DIMENSION_WEIGHTS.documentaryReadiness +
    ...
    // 2. Score >= 80 + sem lacunas críticas = participar
        ...
        overallScore >= DECISION_THRESHOLDS.participar.minScore &&
    ...
    // 3. Score >= 60 com lacunas controláveis = com ressalvas
        ...
        overallScore >= DECISION_THRESHOLDS.ressalvas.minScore &&
...
export function generateActionPlan(matchResult: CompanyEditalMatchResult, assessment: ParticipationAssessment, schemaV2: any): ParticipationActionPlan {
    ...
    for (const risk of matchResult.proposalFit.risks) {
        ...
        if (risk.toLowerCase().includes('desclassifica')) disqualificationRisks.push(risk);
    }
...
function matchTechnical(profile: CompanyLicitationProfile | undefined, schemaV2: any): CompanyEditalMatchResult['technicalFit'] {
    ...
    for (const req of techReqs) {
        ...
        if (hasAttest && (hasArt || !ta.exige_cat)) {
            ...
            missing.push(req.title || req.description);
            if (req.mandatory !== false) highRiskGaps.push(req.title || req.description);
        }
    }
...
function matchEconomicFinancial(profile: CompanyLicitationProfile | undefined, docs: DocumentRecord[], schemaV2: any): CompanyEditalMatchResult['economicFinancialFit'] {
    ...
    if (ef.exige_certidao_negativa_falencia) {
        ...
        if (hasCert) matched.push('Certidão negativa falência/recuperação');
        else missing.push('Certidão negativa falência/recuperação');
    }
...
function matchProposal(profile: CompanyLicitationProfile | undefined, docs: DocumentRecord[], schemaV2: any): CompanyEditalMatchResult['proposalFit'] {
    ...
    if (pa.exige_planilha_orcamentaria) {
        ...
        else missingInputs.push('Planilha orçamentária (OBRIGATÓRIO)');
    }
    if (pa.exige_carta_proposta) {
        if (profile?.readinessFlags.hasProposalTemplates) readyItems.push('Template de carta');
        else missingInputs.push('Carta proposta (OBRIGATÓRIO)');
    }
    ...
    if (pa.criterios_desclassificacao_proposta?.length > 0) {
        risks.push(...pa.criterios_desclassificacao_proposta.map((c: string) => `⚠️ Risco de desclassificação: ${c}`));
    }
    if (pa.criterios_exequibilidade?.length > 0) {
        risks.push(...pa.criterios_exequibilidade.map((c: string) => `📐 Exequibilidade: ${c}`));
    }
...
function generateStrategicNotes(profile: CompanyLicitationProfile | undefined, schemaV2: any): string[] {
    ...
    if (tipoObj && profile.historicalPerformance) {
        ...
        if (best.some(s => tipoObj.toLowerCase().includes(s.toLowerCase()))) notes.push(`✅ Segmento ${tipoObj} é historicamente FORTE para esta empresa (${profile.historicalPerformance.wins}/${profile.historicalPerformance.totalParticipations} vitórias no segmento)`);
        if (worst.some(s => tipoObj.toLowerCase().includes(s.toLowerCase()))) notes.push(`⚠️ Segmento ${tipoObj} é historicamente FRACO para esta empresa — considerar custo-benefício`);
    }
...
function classifyDocArea(docName: string): string {
    ...
    if (lower.includes('contrato social') || lower.includes('juridic') || lower.includes('procuração') || lower.includes('estatuto')) return 'jurídico';
    if (lower.includes('certidão') || lower.includes('fiscal') || lower.includes('cndt') || lower.includes('fgts') || lower.includes('inss') || lower.includes('tribut')) return 'fiscal';
    if (lower.includes('balanço') || lower.includes('patrimônio') || lower.includes('capital') || lower.includes('índice') || lower.includes('falência')) return 'contabilidade';
    if (lower.includes('atestado') || lower.includes('cat') || lower.includes('art') || lower.includes('técnic') || lower.includes('crea') || lower.includes('rrt')) return 'engenharia';
    if (lower.includes('proposta') || lower.includes('planilha') || lower.includes('catálogo') || lower.includes('ficha') || lower.includes('bdi') || lower.includes('cronograma')) return 'comercial';
    if (lower.includes('declaração') || lower.includes('trabalhist')) return 'rh_compliance';
    return 'licitações';
}
