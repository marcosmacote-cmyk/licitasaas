...
export function useProcessForm({ initialData, companies, onClose, onSave, onNavigateToModule }: UseProcessFormOptions) {
    ...
    const nextStep = useMemo(() => {
        ...
        if (stage === 'Captado') {
            if (!hasPdf) return { label: 'Anexar Edital', desc: 'Envie o PDF do edital para triagem', icon: <UploadCloud size={18} />, color: 'var(--color-warning)', action: () => setHubTab('form') };
            return { label: 'Mover para Análise', desc: 'Documento anexado. Avance o processo para iniciar a análise', icon: <ArrowRight size={18} />, color: 'var(--color-primary)', action: undefined };
        }
        if (stage === 'Em Análise' || stage === 'Aprovado para Participação') {
            if (!hasAnalysis && isModuleAllowed(stage, initialData?.substage, 'intelligence')) return { label: 'Analisar com LicitIA', desc: 'Execute a análise do edital para prosseguir', icon: <ScanSearch size={18} />, color: 'var(--color-ai)', action: () => handleAiExtract() };
