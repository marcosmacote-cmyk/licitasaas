...
export class PncpSearchV3 {
    ...
    static async search(input: SearchInput): Promise<SearchResult> {
        ...
        if (input.orgaosLista && input.orgaosLista.trim()) {
            ...
            const orgaoOrConditions: string[] = [];
        ...
        try {
            ...
            logger.info(`[SearchV3] Query: uf=${input.uf || '*'} status=${input.status || '*'} kw=${input.keywords || '-'} page=${page}`);

            // Execute both queries in parallel
            const [countResult, rows] = await Promise.all([
                prisma.$queryRawUnsafe(countSql, ...params.slice(0, countParamIdx - 1)) as Promise<any[]>,
            ...
            logger.info(`[SearchV3] ✅ ${total} results, page ${page}/${Math.ceil(total / pageSize) || 1}, ${elapsed}ms`);
            ...
            const elapsed = Date.now() - start;
            logger.error(`[SearchV3] ❌ FAILED in ${elapsed}ms: ${error?.message}`, {
