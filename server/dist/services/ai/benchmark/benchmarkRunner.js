...
exports.evaluateAgainstBenchmark = evaluateAgainstBenchmark;
...
function evaluateAgainstBenchmark(caseId, analysisOutput) {
    ...
    const foundCategories = new Set(Object.entries(analysisOutput.requirements || {})
        .filter(([_, v]) => Array.isArray(v) && v.length > 0)
        .map(([k]) => k));
    const expectedCategories = expected.categories_to_find || [];
    ...
    const expectedKeyReqs = expected.key_requirements || [];
    const keyReqsFound = expectedKeyReqs.filter(expected => allReqTexts.some(t => t.includes(expected.toLowerCase().substring(0, 15))));
    const keyReqsPct = expectedKeyReqs.length > 0
        ? Math.round((keyReqsFound.length / expectedKeyReqs.length) * 100)
        : 100;
    if (keyReqsPct < 100) {
        const missing = expectedKeyReqs.filter(e => !allReqTexts.some(t => t.includes(e.toLowerCase().substring(0, 15))));
    ...
    const expectedCritical = expected.critical_points || [];
    const criticalFound = expectedCritical.filter(expected => allCriticalText.some(t => t.includes(expected.toLowerCase().replace(/_/g, ' ').substring(0, 10))));
