...
exports.recordAnalysisTelemetry = recordAnalysisTelemetry;
    ...
    try {
        await prisma.aiAnalysisTelemetry.create({
            data: {
                ...
                safetyNetsTriggered: input.safetyNetsTriggered,
                status: input.status,
...
async function getPipelineHealth(periodDays = 7) {
    ...
    const netCounts = {};
    for (const r of records) {
        const nets = r.safetyNetsTriggered || [];
        for (const n of nets) {
...
// ── Drift Detection ──
function checkDriftAlerts(records) {
    const alerts = [];
    ...
    const last3Quality = last3.filter((r) => r.qualityScore != null).map((r) => r.qualityScore);
    if (last3Quality.length === 3 && last3Quality.every((q) => q < 70)) {
        alerts.push({
            type: 'quality_drop',
            severity: 'critical',
            message: `Últimas 3 análises com qualidade < 70%: [${last3Quality.join(', ')}]`,
            value: Math.round(last3Quality.reduce((a, b) => a + b, 0) / 3),
            threshold: 70,
            detectedAt: now,
        });
    }
    // Enforcer overload (average > 25 corrections)
    const avgCorrections = records.reduce((s, r) => s + r.enforcerCorrections, 0) / records.length;
    ...
    const highRepairs = records.filter((r) => r.parseRepairs > 5);
    ...
    const thinCount = records.filter((r) => r.totalRequirements < 10).length;
    ...
    const avgTime = records.reduce((s, r) => s + r.totalTimeMs, 0) / records.length;
    if (avgTime > 300000) {
        alerts.push({
            type: 'slow_pipeline',
            severity: 'warning',
...
function classifySafetyNets(details) {
    const nets = new Set();
    for (const d of details) {
        const dl = d.toLowerCase();
        if (dl.includes('cnpj') && dl.includes('ie') && dl.includes('im'))
            nets.add('RFT_REGRA_OURO');
        else if (dl.includes('rft') && (dl.includes('cnd') || dl.includes('fgts') || dl.includes('cndt')))
            nets.add('RFT_CND_INJECTION');
        else if (dl.includes('qef') && (dl.includes('balanç') || dl.includes('índice') || dl.includes('falência')))
            nets.add('QEF_INJECTION');
        else if (dl.includes('pc:') || dl.includes('pc '))
            nets.add('PC_CLEANUP');
        else if (dl.includes('consórcio'))
            nets.add('HJ_CONSORCIO');
        else if (dl.includes('reserva'))
            nets.add('RFT_DC_RESERVA_PCD');
        else if (dl.includes('dc↔rft') || dl.includes('dc→rft') || dl.includes('duplicado'))
            nets.add('DC_RFT_DEDUP');
        else if (dl.includes('habilitacao_juridica') && dl.includes('vazia'))
            nets.add('HJ_EMPTY_INJECTION');
        else if (dl.includes('participation_conditions'))
            nets.add('PART_COND_DEFAULTS');
        else if (dl.includes('modalidade'))
            nets.add('MODALIDADE_NORMALIZATION');
        else if (dl.includes('description') && dl.includes('copiado'))
            nets.add('DESCRIPTION_COPY');
    }
    return [...nets];
}
