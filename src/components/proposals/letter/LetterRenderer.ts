...
export class LetterRenderer {
    ...
    renderBlock(block: LetterBlock): string {
        ...
        const titleHtml = sectionTitle
            ? `<p style="margin: 3px 0 1px 0; font-weight: bold; font-size: 9.5px; letter-spacing: 0.3px;">${sectionTitle}</p>\n`
            : '';

        switch (block.type) {
            case LetterBlockType.TITLE:
                return `<div class="block block-title" style="margin-bottom: 4px; text-align: center;">
                    <p style="font-weight: bold; font-size: 11.5px; margin: 0; letter-spacing: 0.5px;">${formatted}</p>
                </div>`;

            case LetterBlockType.RECIPIENT:
                return `<div class="block block-recipient" style="margin-bottom: 3px;">
                    <p style="font-weight: bold; margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted.replace(/\n/g, '<br/>')}</p>
                </div>`;

            case LetterBlockType.REFERENCE:
                return `<div class="block block-reference" style="margin-bottom: 2px;">
                    <p style="font-weight: bold; font-size: 9.5px; margin: 0;">${formatted}</p>
                </div>`;

            case LetterBlockType.QUALIFICATION:
                return `<div class="block block-qualification" style="margin-bottom: 2px; text-align: justify;">
                    <p style="margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted.replace(/\n/g, '<br/>')}</p>
                </div>`;

            case LetterBlockType.OBJECT:
                return `<div class="block block-object" style="margin-bottom: 2px; text-align: justify;">
                    ${titleHtml}<p style="margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted.replace(/\n/g, '<br/>')}</p>
                </div>`;

            case LetterBlockType.COMMERCIAL:
                return `<div class="block block-commercial" style="margin-bottom: 2px; text-align: justify;">
                    ${titleHtml}${this.renderParagraphs(formatted)}
                </div>`;

            case LetterBlockType.PRICING_SUMMARY:
                return `${titleHtml}<div class="block block-pricing highlight-box" style="margin: 2px 0 3px 0; padding: 4px 8px; background-color: #fafafa; border-left: 3px solid #222; border-top: 1px solid #e5e5e5; border-right: 1px solid #e5e5e5; border-bottom: 1px solid #e5e5e5; border-radius: 4px;">
                    ${this.renderParagraphs(formatted)}
                </div>`;

            case LetterBlockType.VALIDITY:
                return `<div class="block block-validity" style="margin-bottom: 2px; text-align: justify;">
                    ${titleHtml}<p style="margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted}</p>
                </div>`;

            case LetterBlockType.PROPOSAL_CONDITIONS:
                return `<div class="block block-conditions" style="margin-bottom: 2px; text-align: justify;">
                    ${titleHtml}${this.renderParagraphs(formatted)}
                </div>`;

            case LetterBlockType.EXECUTION:
                return `<div class="block block-execution" style="margin-bottom: 2px; text-align: justify;">
                    ${titleHtml}${this.renderParagraphs(formatted)}
                </div>`;

            case LetterBlockType.BANKING:
                return `<div class="block block-banking" style="margin-bottom: 2px;">
                    ${titleHtml}<p style="margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted.replace(/\n/g, '<br/>')}</p>
                </div>`;

            case LetterBlockType.CLOSING:
                return `<div class="block block-closing" style="margin-bottom: 2px; margin-top: 2px; page-break-inside: avoid;">
                    <div style="text-align: right; margin-bottom: 2px; font-size: 9.5px;">${formatted.split('\n')[0]}</div>
                    <div style="text-align: left; font-size: 9.5px;">${formatted.split('\n').slice(1).join('<br/>').trim()}</div>
                </div>`;
            ...
            case LetterBlockType.DECLARATION_EXTRA:
                return `<div class="block block-declaration-extra" style="margin-bottom: 2px; text-align: justify;">
                    ${this.renderParagraphs(formatted)}
                </div>`;

            default:
                return `<div class="block" style="margin-bottom: 2px;">
                    <p style="margin: 0; line-height: 1.25; font-size: 9.5px;">${formatted.replace(/\n/g, '<br/>')}</p>
                </div>`;
        }
    }
    ...
    private renderSignature(formatted: string): string {
        ...
        const renderSigItem = (section: string, width: string) => {
            ...
            const linesHtml = lines.map(line => {
                ...
                if (/^_{5,}$/.test(line.trim())) {
                    return `<div style="margin: 4px auto 2px; width: 220px; border-top: 1px solid #333;"></div>`;
                }
                return `<div>${line}</div>`;
            }).join('\n');

            return `<div class="sig-item" style="display: inline-block; width: ${width}; vertical-align: top; text-align: center; font-size: 9.5px; line-height: 1.25;">
                ${linesHtml}
            </div>`;
        };
        ...
        return `<div class="block block-signature signature-block" style="margin-top: 6px; text-align: center; page-break-inside: avoid; display: flex; justify-content: space-around; gap: 10px;">
            ${sigHtml}
        </div>`;
    }
    ...
    private renderParagraphs(text: string): string {
        return text.split(/\n\n+/)
            .filter(p => p.trim())
            .map(p => `<p style="margin: 0 0 2px 0; text-align: justify; line-height: 1.25; font-size: 9.5px;">${p.replace(/\n/g, '<br/>')}</p>`)
            .join('\n');
    }
