...
// ── Version Catalog ──
function registerVersion(entry) {
...
function getCurrentVersion(componentName) {
    return versionStore
...
// ── Regression Runner ──
function runRegression(componentName, candidateVersion, baselineVersion, testResults) {
...
function evaluatePromotion(componentName, candidateVersion, baselineVersion, regressionResult, customThresholds) {
    ...
    const criticalRegressions = regressionResult.regressions.filter(r => r.toLowerCase().includes('false_positive') ||
    ...
    const versionEntry = versionStore.find(v => v.componentName === componentName && v.version === candidateVersion);
    if (versionEntry) {
        if (decision === 'promote') {
            versionEntry.status = 'promoted';
            versionEntry.promotedAt = new Date().toISOString();
            versionStore.filter(v => v.componentName === componentName &&
                v.version !== candidateVersion &&
                v.status === 'promoted').forEach(v => v.status = 'archived');
        }
...
function getRegressionHistory(componentName) {
    const items = componentName
...
function registerInitialVersions() {
