...
// ══════════════════════════════════════════════════════════════
// TEMPORARY DEBUG: Inspect composition prices across regimes
// Remove after investigation is complete
// ══════════════════════════════════════════════════════════════
router.get('/debug-composition/:code', async (req: any, res: any) => {
    try {
        const code = req.params.code;
        
        // Find all matching compositions across all SEINFRA databases
        ...
        const result = comps.map(comp => {
            ...
            const items = comp.items.map((ci: any) => {
                const itemPrice = ci.item?.price || ci.auxiliaryComposition?.totalPrice || 0;
                const coef = Number(ci.coefficient) || 0;
                const subtotal = coef * Number(itemPrice);
                calcTotal += subtotal;
                return {
                    code: ci.item?.code || ci.auxiliaryComposition?.code || 'N/A',
                    description: (ci.item?.description || ci.auxiliaryComposition?.description || '').substring(0, 60),
                    type: ci.type || ci.item?.type || '?',
                    unit: ci.item?.unit || ci.auxiliaryComposition?.unit || '?',
                    coefficient: coef,
            ...
            return {
                regime,
                databaseId: comp.database.id,
                version: comp.database.version,
                uf: comp.database.uf,
                description: comp.description,
                unit: comp.unit,
                storedTotalPrice: Number(comp.totalPrice),
                calculatedTotal: Math.round(calcTotal * 10000) / 10000,
                mismatch: Math.abs(Number(comp.totalPrice) - calcTotal) > 0.1 ? `MISMATCH! diff=${(Number(comp.totalPrice) - calcTotal).toFixed(4)}` : 'OK',
                items,
            };
        });
