...
const REVIEW_RULES = {
    ...
    oracle: [
        ...
        { code: 'ORR05', level: 'recommended', reason: 'Vínculo técnico do profissional não confirmado',
            check: (ctx) => (ctx.moduleOutput?.gaps || []).some((g) => {
...
// ── Executor ──
function evaluateHumanReview(moduleName, moduleOutput, schemaV2, qualityScore, userInput, participationScore, hardBlocks) {
    const rules = REVIEW_RULES[moduleName] || [];
    const ctx = { moduleOutput, schemaV2, qualityScore, userInput, participationScore, hardBlocks };
    const triggeredReasons = [];
    const criticalFields = [];
    let maxLevel = 'not_needed';
    let requiredCount = 0;
    ...
    for (const rule of rules) {
        try {
            if (rule.check(ctx)) {
                ...
                if (rule.criticalFields)
                    criticalFields.push(...rule.criticalFields);
