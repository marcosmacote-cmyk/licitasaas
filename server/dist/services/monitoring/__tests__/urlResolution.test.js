...
exports.PLATFORM_SPECS = void 0;
exports.validateLink = validateLink;
exports.shouldAutoEnrichRun = shouldAutoEnrichRun;
...
exports.PLATFORM_SPECS = PLATFORM_SPECS;
...
function validateLink(link, platform) {
    ...
    if (!spec)
        return `Plataforma "${platform}" não conhecida`;
 ...
 */
function shouldAutoEnrichRun(link) {
    ...
    if (!hasPncp)
        return { shouldRun: false, reason: 'Sem link PNCP' };
    ...
    if (!hasPlatformLink)
        return { shouldRun: true, reason: 'Sem domínio monitorável — precisa enriquecer' };
    if (isGeneric)
        return { shouldRun: true, reason: 'Link genérico detectado — AutoEnrich forçado' };
