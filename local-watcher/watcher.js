...
function buildCompraId(proc) {
  ...
  // UASG deve ter 6 dígitos (ex: 70007 → 070007)
  // modalityCode deve ter 2 dígitos (ex: 5 → 05)
...
async function startProcessMonitor(proc) {
  ...
  // Nova abordagem: Acesso Público + Interceptação de Rede
      ...
      const insertMany = db.transaction((msgs) => {
        ...
        for (const msg of msgs) {
          ...
          try {
            ...
            // IGNORE unique constraint errors (already captured)
          }
        }
  ...
  // Navigate to the public process page
  const processUrl = `${COMPRASNET_BASE}/comprasnet-web/public/compras/acompanhamento-compra?compra=${compraId}`;
  try {
    await page.goto(processUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
    console.log(`  ✅ [${proc.processNumber}/${proc.processYear}] Página pública acessada com sucesso.`);
  } catch (err) {
    console.error(`  ❌ Erro ao navegar no processo ${proc.processNumber}/${proc.processYear}: ${err.message}`);
  }

  // Periodic refresh — try to trigger message fetching
...
async function main() {
  ...
  console.log('');
  console.log('🌐 Abrindo navegador em modo headless (Acesso Público)...');

  const browser = await chromium.launch({
    headless: true, // we don't need a visible browser anymore!
    args: [
      '--no-sandbox',
  ...
  console.log('│                                              │');
  console.log('│  • Monitoramento anônimo, sem necessidade de │');
  console.log('│    login ou sessão do Gov.br                 │');
