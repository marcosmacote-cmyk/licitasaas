...
const moduleContextContracts_1 = require("../modules/moduleContextContracts");
const companyProfileService_1 = require("../company/companyProfileService");
const participationEngine_1 = require("./participationEngine");
// Token budgets aproximados para bloco empresarial por módulo
const COMPANY_TOKEN_BUDGET = {
    chat: 1200,
    petition: 600,
    oracle: 800,
    dossier: 800,
...
function buildModuleCompanyBlock(moduleName, profile, companySummary, matchResult) {
    ...
    switch (moduleName) {
        case 'chat':
            ...
            if (matchResult) {
                ...
                if (matchResult.hardBlocks.length > 0)
                    s.push(`🔴 BLOQUEIOS: ${matchResult.hardBlocks.join('; ')}`);
                if (matchResult.technicalFit.highRiskGaps.length > 0)
                    s.push(`⚠️ Lacunas técnicas críticas: ${matchResult.technicalFit.highRiskGaps.join('; ')}`);
                if (matchResult.documentaryFit.missing.length > 0)
                    s.push(`Docs faltantes: ${matchResult.documentaryFit.missing.slice(0, 8).join(', ')}`);
                if (matchResult.strategicNotes.length > 0)
                    s.push(matchResult.strategicNotes.join('\n'));
            }
        ...
        case 'oracle':
            ...
            if (ta.attests?.length > 0)
                s.push(`Atestados registrados (${ta.attests.length}): ${ta.attests.join('; ')}`);
            if (ta.artCatRrt?.length > 0)
                s.push(`ART/CAT/RRT (${ta.artCatRrt.length}): ${ta.artCatRrt.join('; ')}`);
            if (ta.certificates?.length > 0)
                s.push(`Certificados: ${ta.certificates.join('; ')}`);
            if (ta.recurringCapabilities?.length > 0)
                s.push(`Capacidades recorrentes: ${ta.recurringCapabilities.join('; ')}`);
            // RTs
            ...
            if (matchResult) {
                ...
                if (matchResult.technicalFit.highRiskGaps.length > 0)
                    s.push(`⚠️ Lacunas críticas: ${matchResult.technicalFit.highRiskGaps.join('; ')}`);
            }
        ...
        case 'dossier':
            ...
            if (matchResult) {
                if (matchResult.documentaryFit.matched.length > 0)
                    s.push(`✅ Presentes (${matchResult.documentaryFit.matched.length}): ${matchResult.documentaryFit.matched.join(', ')}`);
                if (matchResult.documentaryFit.expired.length > 0)
                    s.push(`⏰ VENCIDOS (${matchResult.documentaryFit.expired.length}): ${matchResult.documentaryFit.expired.join(', ')}`);
                if (matchResult.documentaryFit.missing.length > 0)
                    s.push(`❌ FALTANTES (${matchResult.documentaryFit.missing.length}): ${matchResult.documentaryFit.missing.join(', ')}`);
                if (matchResult.documentaryFit.uncertain.length > 0)
                    s.push(`❓ Incertos (${matchResult.documentaryFit.uncertain.length}): ${matchResult.documentaryFit.uncertain.join(', ')}`);
            }
            else {
        ...
        case 'declaration':
            ...
            if (cd.tradeName)
                s.push(`Nome Fantasia: ${cd.tradeName}`);
            if (cd.companyType)
                s.push(`Porte: ${cd.companyType}`);
            if (cd.headquarters)
                s.push(`Sede: ${cd.headquarters}`);
            break;
        case 'proposal':
            ...
            if (matchResult) {
                if (matchResult.proposalFit.readyItems.length > 0)
                    s.push(`\n✅ Itens prontos: ${matchResult.proposalFit.readyItems.join(', ')}`);
                if (matchResult.proposalFit.missingInputs.length > 0)
                    s.push(`❌ Itens faltantes: ${matchResult.proposalFit.missingInputs.join(', ')}`);
                if (matchResult.proposalFit.risks.length > 0) {
                    s.push(`\n⚠️ RISCOS DE DESCLASSIFICAÇÃO:`);
                    for (const r of matchResult.proposalFit.risks)
                        s.push(`  • ${r}`);
                }
