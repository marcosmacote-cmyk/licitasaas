...
router.post('/analyze', auth_1.authenticateToken, security_1.aiLimiter, async (req, res) => {
    // ── SSE Setup ──
    res.writeHead(200, {
    ...
    try {
        ...
        logger_1.logger.info(`[PNCP-AI] Fetching attachments: ${arquivosUrl}`);
        let arquivos = [];
        try {
            ...
            logger_1.logger.info(`[PNCP-AI] Found ${arquivos.length} attachments`);
        }
        catch (e) {
            logger_1.logger.warn(`[PNCP-AI] Failed to fetch attachments: ${e.message}`);
        }
        ...
        const filteredArquivos = arquivos.filter((arq) => {
            ...
            if (isExcludedByPattern) {
                logger_1.logger.info(`[PNCP-AI] 🚫 Excluído (template/padrão): "${arq.titulo}" (tipo: ${tipoDesc || tipoId})`);
            ...
            if (isOutros && isGenericAnexo && !hasEssentialKeyword) {
                logger_1.logger.info(`[PNCP-AI] 🚫 Excluído (anexo genérico/projeto): "${arq.titulo}" (tipo: ${tipoDesc || tipoId})`);
        ...
        logger_1.logger.info(`[PNCP-AI] 📋 Catálogo completo: ${pncpAttachments.length} arquivos — ${JSON.stringify(purposeCounts)}`);
        logger_1.logger.info(`[PNCP-AI] 📊 Filtro inteligente: ${arquivos.length} anexos → ${filteredArquivos.length} relevantes (${arquivos.length - filteredArquivos.length} excluídos)`);
        ...
        for (const arq of filteredArquivos) {
            ...
            try {
                ...
                logger_1.logger.info(`[PNCP-AI] Downloading: "${fileName}" (tipo: ${arq.tipoDocumentoDescricao || arq.tipoDocumentoId}) from ${fileUrl}`);
                ...
                if (isPdf) {
                    ...
                    if (!pdfPartsFull) {
                        if (bufferSizeKB > MAX_INLINE_FILE_KB) {
                            ...
                            logger_1.logger.info(`[PNCP-AI] ⚡ Arquivo grande (${Math.round(bufferSizeKB)}KB > ${MAX_INLINE_FILE_KB}KB). Usando Gemini Files API para upload...`);
                            try {
                                const apiKey = process.env.GEMINI_API_KEY;
                                if (!apiKey)
                                    throw new Error('GEMINI_API_KEY não configurada');
                                const filesAi = new genai_1.GoogleGenAI({ apiKey });
                                const tempFilePath = path_1.default.join(files_service_1.uploadDir, `temp_upload_${Date.now()}_${fileName.replace(/[^a-z0-9._-]/gi, '_')}`);
                                ...
                                if (uploadedFile && uploadedFile.uri) {
                                    ...
                                    logger_1.logger.info(`[PNCP-AI] ✅ Upload via Files API concluído: ${uploadedFile.name} (URI: ${uploadedFile.uri})`);
                                }
                                else {
                                    logger_1.logger.warn(`[PNCP-AI] ⚠️ Files API não retornou URI para ${fileName}`);
                                }
                            }
                            catch (e) {
                                logger_1.logger.warn(`[PNCP-AI] ⚠️ Falha no upload via Files API para ${fileName}:`, e.message);
                            }
                            ...
                            if (totalPdfSizeAccum + bufferSizeKB > MAX_TOTAL_PDF_SIZE_KB && pdfParts.length > 0) {
                                logger_1.logger.info(`[PNCP-AI] ⚡ Orçamento inline de ${MAX_TOTAL_PDF_SIZE_KB}KB atingido. Enviando "${fileName}" via Files API...`);
                                try {
                                    const apiKey = process.env.GEMINI_API_KEY;
                                    if (!apiKey)
                                        throw new Error('GEMINI_API_KEY não configurada');
                                    const filesAi = new genai_1.GoogleGenAI({ apiKey });
                                    const tempPath = path_1.default.join(files_service_1.uploadDir, `temp_overflow_${Date.now()}_${fileName.replace(/[^a-z0-9._-]/gi, '_')}`);
                                    ...
                                    if (uploadedFile && uploadedFile.uri) {
                                        pdfParts.push((0, genai_1.createPartFromUri)(uploadedFile.uri, uploadedFile.mimeType || 'application/pdf'));
                                        logger_1.logger.info(`[PNCP-AI] ✅ Overflow via Files API: ${uploadedFile.name}`);
                                    }
                                }
                                catch (e) {
                                    logger_1.logger.warn(`[PNCP-AI] ⚠️ Files API overflow falhou para ${fileName}:`, e.message);
                        ...
                        logger_1.logger.info(`[PNCP-AI] 📁 Salvando "${fileName}" (${Math.round(bufferSizeKB)}KB) apenas no storage (limite de ${MAX_PDF_PARTS} docs para IA atingido)`);
                    }
                    const safeFileName = `pncp_${req.user.tenantId}_${fileName.replace(/[^a-z0-9._-]/gi, '_')}`;
                    fs_1.default.writeFileSync(path_1.default.join(files_service_1.uploadDir, safeFileName), buffer);
                    ...
                    try {
                        ...
                        logger_1.logger.error(`[PNCP-AI] Erro upload PDF Storage:`, e);
                    }
                    // Note: pdfParts is pushed either as text or inlineData above
                    downloadedFiles.push(storageFileName);
                    logger_1.logger.info(`[PNCP-AI] ✅ PDF: ${fileName} saved as ${storageFileName} (${(buffer.length / 1024).toFixed(0)} KB)`);
                }
                else if (isZip) {
                    logger_1.logger.info(`[PNCP-AI] 📦 ZIP detected: ${fileName} (${(buffer.length / 1024).toFixed(0)} KB) — extracting PDFs...`);
                    try {
                        ...
                        let zipEntries = Object.keys(zip.files).filter((name) => {
                            ...
                            if (excluded) {
                                logger_1.logger.info(`[PNCP-AI] 🚫 ZIP: Excluído "${name}" (padrão filtrado)`);
                        ...
                        logger_1.logger.info(`[PNCP-AI] ZIP contains ${zipEntries.length} PDF(s) (sorted): ${zipEntries.join(', ')}`);
                        for (const entryName of zipEntries) {
                            ...
                            if (!pdfPartsFull) {
                                if (entrySizeKB > MAX_SINGLE_FILE_KB) {
                                    logger_1.logger.info(`[PNCP-AI] ⚡ ZIP Entry grande (${Math.round(entrySizeKB)}KB), usando Gemini Files API...`);
                                    try {
                                        const apiKey = process.env.GEMINI_API_KEY;
                                        if (!apiKey)
                                            throw new Error('GEMINI_API_KEY não configurada');
                                        const filesAi = new genai_1.GoogleGenAI({ apiKey });
                                        const tempPath = path_1.default.join(files_service_1.uploadDir, `temp_zip_${Date.now()}_${entryName.replace(/[^a-z0-9._-]/gi, '_')}`);
                                        ...
                                        if (uploadedFile && uploadedFile.uri) {
                                            pdfParts.push((0, genai_1.createPartFromUri)(uploadedFile.uri, uploadedFile.mimeType || 'application/pdf'));
                                            logger_1.logger.info(`[PNCP-AI] ✅ ZIP Entry via Files API: ${uploadedFile.name}`);
                                        }
                                    }
                                    catch (e) {
                                        logger_1.logger.warn(`[PNCP-AI] ⚠️ Falha Files API para ZIP entry ${entryName}:`, e.message);
                                    }
                                    totalPdfSizeAccum += 1;
                                }
                                else {
                                    if (pdfBuffer.length > 0) {
                                        if (totalPdfSizeAccum + entrySizeKB > MAX_TOTAL_PDF_SIZE_KB && pdfParts.length > 0) {
                                            logger_1.logger.warn(`[PNCP-AI] \u26a0\ufe0f Orçamento de ${MAX_TOTAL_PDF_SIZE_KB}KB atingido. Apenas salvando no disco ZIP entry "${entryName}" (${Math.round(entrySizeKB)}KB)`);
                                        }
                                        else {
                                ...
                                logger_1.logger.info(`[PNCP-AI] 📁 Salvando "${entryName}" (${Math.round(entrySizeKB)}KB) do ZIP apenas no storage (limite da IA atingido)`);
                            }
                            if (pdfBuffer.length > 0) {
                                const safeName = `pncp_${req.user.tenantId}_${entryName.replace(/[^a-z0-9._-]/gi, '_')}`;
                                fs_1.default.writeFileSync(path_1.default.join(files_service_1.uploadDir, safeName), pdfBuffer);
                                ...
                                try {
                                    ...
                                    logger_1.logger.error(`[PNCP-AI] Erro upload ZIP-PDF Storage:`, e);
                                }
                                downloadedFiles.push(storageFileName);
                                logger_1.logger.info(`[PNCP-AI] ✅ Extracted from ZIP: ${entryName} saved as ${storageFileName} (${(pdfBuffer.length / 1024).toFixed(0)} KB)`);
                            }
                        }
                    }
                    catch (zipErr) {
                        logger_1.logger.warn(`[PNCP-AI] Failed to extract ZIP ${fileName}: ${zipErr.message}`);
                    }
                }
                else if (isRar) {
                    logger_1.logger.info(`[PNCP-AI] 📦 RAR detected: ${fileName} (${(buffer.length / 1024).toFixed(0)} KB) — extracting PDFs...`);
                    try {
                        ...
                        const pdfFiles = files.filter(f => {
                            ...
                            const excluded = ARCHIVE_EXCLUDE_PATTERNS.some(pat => n.includes(pat));
                            if (excluded) {
                                logger_1.logger.info(`[PNCP-AI] 🚫 RAR: Excluído "${f.fileHeader.name}" (padrão filtrado)`);
                        ...
                        logger_1.logger.info(`[PNCP-AI] RAR contains ${pdfFiles.length} PDF(s) (sorted): ${pdfFiles.map(f => f.fileHeader.name).join(', ')}`);
                        for (const rarFile of pdfFiles) {
                            ...
                            if (rarFile.extraction && rarFile.extraction.length > 0) {
                                ...
                                if (!pdfPartsFull) {
                                    if (entrySizeKB > MAX_SINGLE_FILE_KB) {
                                        logger_1.logger.info(`[PNCP-AI] ⚡ RAR Entry grande (${Math.round(entrySizeKB)}KB), usando Gemini Files API...`);
                                        try {
                                            const apiKey = process.env.GEMINI_API_KEY;
                                            if (!apiKey)
                                                throw new Error('GEMINI_API_KEY não configurada');
                                            const filesAi = new genai_1.GoogleGenAI({ apiKey });
                                            const tempPath = path_1.default.join(files_service_1.uploadDir, `temp_rar_${Date.now()}_${rarFile.fileHeader.name.replace(/[^a-z0-9._-]/gi, '_')}`);
                                            fs_1.default.writeFileSync(tempPath, pdfBuffer);
                                            const uploadedFile = await filesAi.files.upload({
                                                file: tempPath,
                                            ...
                                            if (uploadedFile && uploadedFile.uri) {
                                                pdfParts.push((0, genai_1.createPartFromUri)(uploadedFile.uri, uploadedFile.mimeType || 'application/pdf'));
                                                logger_1.logger.info(`[PNCP-AI] ✅ RAR Entry via Files API: ${uploadedFile.name}`);
                                            }
                                        }
                                        catch (e) {
                                            logger_1.logger.warn(`[PNCP-AI] ⚠️ Falha Files API para RAR entry ${rarFile.fileHeader.name}:`, e.message);
                                        }
                                        totalPdfSizeAccum += 1;
                                    }
                                    else {
                                        if (totalPdfSizeAccum + entrySizeKB > MAX_TOTAL_PDF_SIZE_KB && pdfParts.length > 0) {
                                            logger_1.logger.warn(`[PNCP-AI] ⚠️ Orçamento de ${MAX_TOTAL_PDF_SIZE_KB}KB atingido. Apenas salvando no disco RAR entry "${rarFile.fileHeader.name}" (${Math.round(entrySizeKB)}KB)`);
                                        }
                                        else {
                                            totalPdfSizeAccum += entrySizeKB;
                                            pdfParts.push({
                                    ...
                                    logger_1.logger.info(`[PNCP-AI] 📁 Salvando "${rarFile.fileHeader.name}" (${Math.round(entrySizeKB)}KB) do RAR apenas no storage (limite da IA atingido)`);
                                }
                                ...
                                try {
                                    ...
                                    logger_1.logger.error(`[PNCP-AI] Erro upload RAR-PDF Storage:`, e);
                                }
                                downloadedFiles.push(storageFileName);
                                logger_1.logger.info(`[PNCP-AI] ✅ Extracted from RAR: ${rarFile.fileHeader.name} (${(pdfBuffer.length / 1024).toFixed(0)} KB)`);
                            }
                        }
                    }
                    catch (rarErr) {
                        logger_1.logger.warn(`[PNCP-AI] Failed to extract RAR ${fileName}: ${rarErr.message}`);
                    }
                }
                else {
                    logger_1.logger.info(`[PNCP-AI] ⏭️ Skipped non-PDF/non-ZIP/non-RAR: ${fileName} (first bytes: ${buffer[0].toString(16)} ${buffer[1].toString(16)})`);
                }
            }
            catch (dlErr) {
                logger_1.logger.warn(`[PNCP-AI] Failed to download ${fileName}: ${dlErr.message}`);
            }
        }
        ...
        logger_1.logger.info(`[PNCP-V2] 🤖 Modelos: E1=${PIPELINE_MODELS.extraction} | E2=${PIPELINE_MODELS.normalization} (QTP=${PIPELINE_MODELS.normQtp}) | E3=${PIPELINE_MODELS.riskReview}`);
        ...
        logger_1.logger.info(`[PNCP-V2] ═══ PIPELINE INICIADO ═══ (${pdfParts.length} PDFs, ${downloadedFiles.join(', ')})`);
        ...
        logger_1.logger.info(`[PNCP-V2] ── Etapa 1/3: Extração Factual (${pdfParts.length} partes, ${Math.round(totalPdfSizeKB)}KB inline — ${pdfSizes.join(', ')})...`);
        let extractionJson;
        ...
        try {
            ...
            logger_1.logger.info(`[PNCP-V2] ✅ Etapa 1 em ${stageTimes.extraction.toFixed(1)}s — ${(extractionJson.evidence_registry || []).length} evidências, ${Object.values(extractionJson.requirements || {}).flat().length} exigências`);
        }
        catch (err) {
            ...
            logger_1.logger.warn(`[PNCP-V2] ⚠️ Etapa 1 Gemini falhou (${isServiceOverload ? 'SOBRECARGA' : 'ERRO'}): ${errMsg}. Tentando OpenAI...`);
            ...
            try {
                ...
                stageTimes.extraction = (Date.now() - t1Start) / 1000;
                logger_1.logger.info(`[PNCP-V2] ✅ Etapa 1 via OpenAI em ${stageTimes.extraction.toFixed(1)}s`);
            }
            catch (openAiErr) {
                logger_1.logger.error(`[PNCP-V2] ❌ Etapa 1 falhou (ambos modelos)`);
        ...
        logger_1.logger.info(`[PNCP-V2] 📋 itens_licitados: ${Array.isArray(extractedItens) ? extractedItens.length : 0} itens extraídos pela Etapa 1`);
        ...
        if (injectedCount > 0) {
            ...
            logger_1.logger.info(`[PNCP-V2] 🔧 RFT completude: +${injectedCount} doc(s) injetado(s) (CNPJ/inscrições omitidos pela IA)`);
        }
        ...
        if (hasIE && hasIM) {
            ...
            if (dedupedRft.length < beforeLen) {
                ...
                logger_1.logger.info(`[PNCP-V2] 🧹 Dedup IE/IM: removido(s) ${beforeLen - dedupedRft.length} item(ns) genérico(s) (IE+IM separados existem)`);
            }
        }
        ...
        if (extractionJson.requirements) {
            const catCounts = Object.entries(extractionJson.requirements)
                ...
                .join(' | ');
            logger_1.logger.info(`[PNCP-V2] 📋 Exigências por categoria: ${catCounts}`);
        }
        logger_1.logger.info(`[PNCP-V2] 📊 Extração: ${extractedReqs} exigências, ${extractedEvidence} evidências, processo=${hasProcessId}`);
        ...
        if (hallucinationSignals.length > 0) {
            logger_1.logger.error(`[PNCP-V2] 🚨 ALUCINAÇÃO DETECTADA: ${hallucinationSignals.join(', ')}`);
            logger_1.logger.error(`[PNCP-V2] 🚨 A IA gerou dados de TEMPLATE em vez de ler o PDF real. Abortando análise.`);
        ...
        if (extractedReqs < MIN_REQUIREMENTS && extractedEvidence < MIN_EVIDENCE && !hasProcessId) {
            logger_1.logger.error(`[PNCP-V2] ❌ FALHA FACTUAL DURA: ${extractedReqs} exigências (mín: ${MIN_REQUIREMENTS}), ${extractedEvidence} evidências (mín: ${MIN_EVIDENCE}), sem identificação do processo`);
            v2Result.analysis_meta.workflow_stage_status.extraction = 'failed';
        ...
        if (extractedReqs < MIN_REQUIREMENTS || extractedEvidence < MIN_EVIDENCE) {
            logger_1.logger.warn(`[PNCP-V2] ⚠️ Extração abaixo do ideal: ${extractedReqs} exigências, ${extractedEvidence} evidências — pipeline continua com degradação`);
        ...
        if (domainReinforcement) {
            logger_1.logger.info(`[PNCP-V2] 🎯 Roteamento por tipo: ${detectedObjectType}`);
        }
        ...
        if (shouldReExtract) {
            logger_1.logger.warn(`[PNCP-V2] 🔍 GAP DETECTADO: ${missingCategories.length} categorias vazias para ${objType}: ${missingCategories.join(', ')} ` +
            ...
            logger_1.logger.info(`[PNCP-V2] ── Re-extração focada para categorias faltantes...`);
...
Retorne JSON com: { "requirements": { ... apenas categorias faltantes ... }, "evidence_registry": [...] }`;
            ...
            try {
                ...
                if (reText) {
                        ...
                        for (const [cat, items] of Object.entries(reData.requirements)) {
                            if (Array.isArray(items) && items.length > 0) {
                                ...
                                if (existing.length === 0) {
                                    ...
                                    logger_1.logger.info(`[PNCP-V2] ✅ Re-extração ${cat}: +${items.length} itens`);
                                }
                                    ...
                                    if (newItems.length > 0) {
                                        ...
                                        logger_1.logger.info(`[PNCP-V2] ✅ Re-extração RFT (CNDs): +${newItems.length} itens adicionais`);
                                    }
                                }
                            }
                        }
                    ...
                    logger_1.logger.info(`[PNCP-V2] ✅ Re-extração concluída em ${reDuration}s: +${reExtractedCount} exigências, +${(reData.evidence_registry || []).length} evidências`);
                ...
                const reDuration = ((Date.now() - t15Start) / 1000).toFixed(1);
                logger_1.logger.warn(`[PNCP-V2] ⚠️ Re-extração falhou em ${reDuration}s: ${reErr.message}. Continuando com dados parciais.`);
            ...
            logger_1.logger.info(`[PNCP-V2] ℹ️ ${missingCategories.length} categorias vazias (${missingCategories.join(', ')}) — sem sinal de truncamento, mantendo extração original`);
        }
        ...
        logger_1.logger.info(`[PNCP-V2] ── Etapas 2+3/3: Normalização + Risco (paralelo)...`);
        ...
        const [normSettled, riskSettled, itemsSettled] = await Promise.allSettled([
            ...
            (async () => {
                ...
                const categoryTasks = prompt_service_1.NORM_CATEGORIES.map(cat => {
                    ...
                    if (FAST_NORM_CATEGORIES.includes(cat.key)) {
                        ...
                        logger_1.logger.info(`[PNCP-V2] ⚡ FastNorm ${cat.prefix}: ${normalized.length} itens (server-side, 0 API calls)`);
                    ...
                    return (async () => {
                        ...
                        try {
                            ...
                            logger_1.logger.info(`[PNCP-V2] ✅ Norm ${cat.prefix}: ${(data.items || []).length} itens (${normModel})`);
                            return { cat: cat.key, success: true };
                        }
                        catch (geminiErr) {
                            // Per-block fallback to OpenAI
                            logger_1.logger.warn(`[PNCP-V2] ⚠️ Norm ${cat.prefix} Gemini falhou: ${geminiErr.message}. Fallback OpenAI...`);
                            usedFallback = true;
                            try {
                                ...
                                logger_1.logger.info(`[PNCP-V2] ✅ Norm ${cat.prefix} via OpenAI: ${(data.items || []).length} itens`);
                                return { cat: cat.key, success: true, fallback: true };
                            }
                            catch (oaiErr) {
                                // Both failed — keep original extraction data
                                logger_1.logger.error(`[PNCP-V2] ❌ Norm ${cat.prefix} falhou (ambos): ${oaiErr.message}`);
                                mergedRequirements[cat.key] = items;
                                totalNormalized += items.length;
                                categoriesFailed++;
                ...
                logger_1.logger.info(`[PNCP-V2] ✅ Etapa 2 em ${stageTimes.normalization.toFixed(1)}s — ${totalNormalized} itens normalizados, ${successCount}/${prompt_service_1.NORM_CATEGORIES.length - categoriesSkipped} OK (⚡${fastPathCount} fast + 🤖${aiNormCount} AI), ${categoriesFailed} falhas`);
            ...
            (async () => {
                ...
                try {
                    ...
                    logger_1.logger.info(`[PNCP-V2] ✅ Etapa 3 em ${stageTimes.risk_review.toFixed(1)}s — ${(json.legal_risk_review?.critical_points || []).length} pontos críticos`);
                    return { json, model: PIPELINE_MODELS.riskReview, repaired: parseR.repaired, fallback: false };
                }
                catch (err) {
                    logger_1.logger.warn(`[PNCP-V2] ⚠️ Etapa 3 Gemini falhou: ${err.message}. Tentando OpenAI...`);
                    const openAiResult = await (0, openai_service_1.fallbackToOpenAiV2)({
                    ...
                    stageTimes.risk_review = (Date.now() - t3Start) / 1000;
                    logger_1.logger.info(`[PNCP-V2] ✅ Etapa 3 via OpenAI em ${stageTimes.risk_review.toFixed(1)}s`);
            ...
            (async () => {
                ...
                if (Array.isArray(currentItens) && currentItens.length > 0) {
                    logger_1.logger.info(`[PNCP-V2] ⚡ Etapa 1.5 SKIP — itens_licitados já tem ${currentItens.length} itens`);
                ...
                if (planilhaAttachments.length === 0) {
                    logger_1.logger.info(`[PNCP-V2] ⚡ Etapa 1.5 SKIP — sem planilhas no catálogo`);
                    return { items: [], skipped: true };
                }
                logger_1.logger.info(`[PNCP-V2] 📋 Etapa 1.5: Extraindo itens de ${planilhaAttachments.length} PDF(s) em paralelo...`);
                const t15Start = Date.now();
                try {
                    ...
                    logger_1.logger.info(`[PNCP-V2] 📋 Etapa 1.5: PDF "${target.titulo}" (${(pdfBuffer.length / 1024).toFixed(0)} KB)`);
...
[{"itemNumber":"1","description":"...","unit":"UN","quantity":1,"referencePrice":0,"multiplier":1,"multiplierLabel":""}]`;
                    ...
                    logger_1.logger.info(`[PNCP-V2] ✅ Etapa 1.5 em ${elapsed}s — ${items.length} itens extraídos de "${target.titulo}"`);
                    return { items, skipped: false, source: target.titulo, elapsed };
                }
                catch (err) {
                    logger_1.logger.warn(`[PNCP-V2] ⚠️ Etapa 1.5 falhou: ${err.message}`);
                    return { items: [], skipped: false, error: err.message };
                }
            })()
        ]);
        logger_1.logger.info(`[PNCP-V2] Etapas 2+3 paralelas concluídas em ${((Date.now() - t2t3Start) / 1000).toFixed(1)}s`);
        ...
        if (normSettled.status === 'fulfilled') {
            ...
            logger_1.logger.error(`[PNCP-V2] ❌ Etapa 2 falhou — continuando sem normalização`);
        ...
        if (riskSettled.status === 'fulfilled') {
            ...
            logger_1.logger.error(`[PNCP-V2] ❌ Etapa 3 falhou — continuando sem revisão de risco`);
        ...
        if (itemsSettled.status === 'fulfilled' && !itemsSettled.value.skipped) {
            ...
            if (extractedItems.length > 0) {
                ...
                logger_1.logger.info(`[PNCP-V2] ✅ Etapa 1.5 merge: ${extractedItems.length} itens → proposal_analysis.itens_licitados`);
            }
        }
        else if (itemsSettled.status === 'rejected') {
            logger_1.logger.warn(`[PNCP-V2] ⚠️ Etapa 1.5 rejected: ${itemsSettled.reason?.message || 'erro'}`);
        }
        ...
        // ── Validation (no AI) ──
        const validation = (0, analysisQualityEvaluator_1.validateAnalysisCompleteness)(v2Result);
        ...
        try {
            ...
            logger_1.logger.warn(`[PNCP-V2] Motor de regras falhou: ${ruleErr.message}`);
        }
        // ── Quality Evaluator ──
        ...
        try {
            ...
            logger_1.logger.warn(`[PNCP-V2] Avaliador de qualidade falhou: ${qualErr.message}`);
        }
        ...
        logger_1.logger.info(`[PNCP-V2] ═══ PIPELINE CONCLUÍDO ═══ ${totalDuration}s total | ` +
        ...
        try {
            ...
            if (d) {
                ...
                logger_1.logger.info(`[PNCP-V2] 💰 API metadata: valor=${pncpApiValue}, sessionDate=${pncpApiSessionDate || '(vazio)'}`);
            }
        }
        catch (e) {
            logger_1.logger.warn(`[PNCP-V2] Failed to fetch PNCP metadata for value: ${e.message}`);
        }
        ...
        logger_1.logger.info(`[PNCP-V2] 💰 Valor resolução: AI=${aiExtractedValue}, API=${pncpApiValue}, final=${resolvedEstimatedValue}`);
        ...
        if (rawObjResumo !== bestObjResumo) {
            logger_1.logger.info(`[PNCP-V2] 🧹 Sanitização anti-Minuta: obj "${rawObjResumo.slice(0, 50)}..." → "${bestObjResumo.slice(0, 50)}..."`);
        }
        const legacyProcess = {
            ...
            modality: (0, biddingHelpers_1.normalizeModality)(v2Result.process_identification.modalidade),
            portal: (0, biddingHelpers_1.normalizePortal)(v2Result.process_identification.fonte_oficial || 'PNCP', link_sistema),
            ...
            link_sistema: (() => {
                ...
                if (isGenericComprasNet) {
                    logger_1.logger.info(`[PNCP-V2] 🧹 Sanitização: link_sistema genérico removido: "${rawLink.substring(0, 60)}"`);
                    return '';
                }
                return rawLink;
            })()
        ...
        if (needsAutoEnrich) {
            try {
                ...
                logger_1.logger.info(`[PNCP-V2] 🔍 Buscando linkSistemaOrigem: ${enrichUrl} (link_sistema=${legacyProcess.link_sistema ? 'genérico' : 'vazio'})`);
                ...
                if (enrichRes.ok) {
                    ...
                    if (lso && (0, biddingHelpers_1.hasMonitorableDomain)(lso)) {
                        legacyProcess.link_sistema = lso;
                        const platform = (0, biddingHelpers_1.detectPlatformFromLink)(lso) || 'desconhecida';
                        logger_1.logger.info(`[PNCP-V2] ✅ linkSistemaOrigem enriquecido (${platform}): ${lso.substring(0, 80)}`);
                    }
                    else {
                        logger_1.logger.info(`[PNCP-V2] ⚠️ linkSistemaOrigem=${lso ? lso.substring(0, 60) : 'VAZIO'} → tentando Fallback B (edital)`);
                        ...
                        try {
                                    ...
                                    if (pdfBuffer) {
                                        ...
                                        if (!nuCompraRaw) {
                                            ...
                                            if (pdfNumMatch) {
                                                ...
                                                logger_1.logger.info(`[PNCP-V2] 📄 Fallback C: numero_comprasnet=${nuCompraRaw} extraído do PDF direto`);
                                            }
                                        }
                                        if (!resolvedUasg) {
                                            ...
                                            if (pdfUasgMatch) {
                                                ...
                                                logger_1.logger.info(`[PNCP-V2] 📄 Fallback C: uasg=${resolvedUasg} extraído do PDF direto`);
                                            }
                                        }
                                    }
                                }
                                catch (pdfErr) {
                                    logger_1.logger.warn(`[PNCP-V2] ⚠️ Fallback C (pdf-parse) falhou: ${pdfErr.message}`);
                                }
                            }
                            ...
                            if (nuCompraRaw && coModalidade && resolvedUasg && resolvedUasg.length === 6) {
                                ...
                                logger_1.logger.info(`[PNCP-V2] 🔧 Fallback B: URL construída do edital → ${fallbackUrl}`);
                                logger_1.logger.info(`[PNCP-V2]    UASG=${resolvedUasg} mod=${coModalidade} num=${nuCompra} ano=${compraAno} src=${extractionSrc}`);
                            }
                            else {
                                logger_1.logger.info(`[PNCP-V2] ℹ️ Fallback B+C: dados insuficientes (nuCompra=${nuCompraRaw || 'N/A'}, coMod=${coModalidade || 'N/A'}, uasg=${resolvedUasg || 'N/A'})`);
                            }
                        }
                        catch (fbErr) {
                            logger_1.logger.warn(`[PNCP-V2] ⚠️ Fallback B falhou: ${fbErr.message}`);
                        }
                    }
                }
            }
            catch (err) {
                logger_1.logger.warn(`[PNCP-V2] ⏱️ Enrich falhou: ${err.message}`);
            }
        }
        ...
        if (legacyProcess.link_sistema && (0, biddingHelpers_1.hasMonitorableDomain)(legacyProcess.link_sistema)) {
            const enrichedPortal = (0, biddingHelpers_1.normalizePortal)(legacyProcess.portal || 'PNCP', legacyProcess.link_sistema);
            if (enrichedPortal !== legacyProcess.portal) {
                logger_1.logger.info(`[PNCP-V2] 🔄 Portal re-normalizado: "${legacyProcess.portal}" → "${enrichedPortal}" (Auto-Enrich)`);
        ...
        logger_1.logger.error('[PNCP-V2] Error:', error?.message || error);
