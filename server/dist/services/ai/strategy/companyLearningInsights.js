...
// ── Storage ──
const matchHistoryStore = new Map();
function recordMatchHistory(companyId, processId, scores, recommendation) {
    if (!matchHistoryStore.has(companyId))
        matchHistoryStore.set(companyId, []);
...
function generateCompanyInsights(companyId) {
    ...
    const catOrder = { weakness: 0, pattern: 1, trend: 2, opportunity: 3, strength: 4 };
...
function buildReadinessProfile(profile, history) {
    if (!profile)
        return { documentaryReadiness: 'weak', technicalReadiness: 'weak', economicFinancialReadiness: 'weak', overallReadiness: 'weak' };
