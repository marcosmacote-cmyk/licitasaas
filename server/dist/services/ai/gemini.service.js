 ...
 */
async function callGeminiWithRetry(model, options, maxRetries = 2, trackingContext) {
    let lastError;
    ...
    const executeCall = async () => {
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                const result = await withTimeout(model.generateContent({ ...options, model: requestedModel }), TIMEOUT_MS, `${requestedModel} attempt ${i + 1}`);
                return result;
            }
            catch (error) {
                lastError = error;
