...
router.get('/:biddingId', auth_1.authenticateToken, async (req, res) => {
    try {
        const proposals = await prisma_1.prisma.priceProposal.findMany({
        ...
        logger_1.logger.error('[Proposals] GET error:', error.message);
        res.status(500).json({ error: 'Failed to fetch proposals' });
    }
});
// GET single proposal with items
router.get('/detail/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const proposal = await prisma_1.prisma.priceProposal.findFirst({
        ...
        logger_1.logger.error('[Proposals] GET detail error:', error.message);
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const company = await prisma_1.prisma.companyProfile.findUnique({ where: { id: companyProfileId } });
        // Count existing versions
        const existingCount = await prisma_1.prisma.priceProposal.count({
            where: { biddingProcessId, tenantId: req.user.tenantId },
        });
        const proposal = await prisma_1.prisma.priceProposal.create({
            data: {
        ...
        logger_1.logger.info(`[Proposals] Created proposal ${proposal.id} v${proposal.version} for bidding ${biddingProcessId}`);
        res.status(201).json(proposal);
    }
    catch (error) {
        logger_1.logger.error('[Proposals] POST error:', error.message);
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existing = await prisma_1.prisma.priceProposal.findFirst({
            where: { id: req.params.id, tenantId: req.user.tenantId },
        });
        if (!existing)
            return res.status(404).json({ error: 'Proposal not found' });
        const updated = await prisma_1.prisma.priceProposal.update({
        ...
        await prisma_1.prisma.priceProposal.update({ where: { id: req.params.id }, data: { totalValue } });
        updated.totalValue = totalValue;
        res.json(updated);
    }
    catch (error) {
        logger_1.logger.error('[Proposals] PUT error:', error.message);
...
router.delete('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'Proposal not found' });
        await prisma_1.prisma.priceProposal.delete({ where: { id: req.params.id } });
        res.json({ success: true });
    }
    catch (error) {
        logger_1.logger.error('[Proposals] DELETE error:', error.message);
...
router.post('/:id/items', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (replaceAll) {
            await prisma_1.prisma.proposalItem.deleteMany({ where: { proposalId } });
        }
        // Create items
        const created = [];
        for (let i = 0; i < items.length; i++) {
            ...
            const dbItem = await prisma_1.prisma.proposalItem.create({
                data: {
                    proposalId,
        ...
        // Recalculate totals
        const allItems = await prisma_1.prisma.proposalItem.findMany({ where: { proposalId } });
        ...
        await prisma_1.prisma.priceProposal.update({ where: { id: proposalId }, data: { totalValue, ...(adjustedTotalValue > 0 ? { adjustedTotalValue } : {}) } });
        logger_1.logger.info(`[Proposals] Added ${created.length} items to proposal ${proposalId}, rounding: ${useRounding}, total: R$ ${totalValue.toFixed(2)}${adjustedTotalValue > 0 ? `, adjusted: R$ ${adjustedTotalValue.toFixed(2)}` : ''}`);
        res.json({ items: created, totalValue });
    }
    catch (error) {
        logger_1.logger.error('[Proposals] POST items error:', error.message);
        res.status(500).json({ error: 'Failed to add items' });
    }
});
// PUT update single item
router.put('/:id/items/:itemId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const updated = await prisma_1.prisma.proposalItem.update({
            where: { id: itemId },
            data: {
        ...
        const allItems = await prisma_1.prisma.proposalItem.findMany({ where: { proposalId } });
        const totalValue = allItems.reduce((sum, it) => sum + (it.totalPrice || 0), 0);
        await prisma_1.prisma.priceProposal.update({ where: { id: proposalId }, data: { totalValue } });
        res.json({ item: updated, totalValue });
    }
    catch (error) {
        logger_1.logger.error('[Proposals] PUT item error:', error.message);
...
router.delete('/:id/items/:itemId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (!proposal)
            return res.status(404).json({ error: 'Proposal not found' });
        await prisma_1.prisma.proposalItem.delete({ where: { id: req.params.itemId } });
        // Recalculate total
        const allItems = await prisma_1.prisma.proposalItem.findMany({ where: { proposalId } });
        const totalValue = allItems.reduce((sum, it) => sum + (it.totalPrice || 0), 0);
        await prisma_1.prisma.priceProposal.update({ where: { id: proposalId }, data: { totalValue } });
        ...
        logger_1.logger.error('[Proposals] DELETE item error:', error.message);
        res.status(500).json({ error: 'Failed to delete item' });
    }
});
// POST AI Populate — extract items from AI analysis
router.post('/ai-populate', auth_1.authenticateToken, async (req, res) => {
    try {
        const { biddingProcessId } = req.body;
        // Get bidding with AI analysis
        const bidding = await prisma_1.prisma.biddingProcess.findFirst({
        ...
        if (Array.isArray(itensLicitados) && itensLicitados.length > 0) {
            logger_1.logger.info(`[AI Populate] ✅ Strategy 0: Using ${itensLicitados.length} pre-extracted items from schemaV2`);
        ...
        if (hasRealBiddingItems) {
            logger_1.logger.info(`[AI Populate] Using legacy biddingItems (${biddingItems.length} chars)`);
...
[{"itemNumber":"1","description":"Descrição completa","unit":"Mês","quantity":3,"multiplier":12,"multiplierLabel":"Meses","referencePrice":22465.00}]`;
            ...
            if (isSuspiciouslyFew) {
                logger_1.logger.warn(`[AI Populate] ⚠️ Strategy 1 returned only ${items.length} items but estimatedValue=R$${estimatedValue.toLocaleString()} — likely truncated biddingItems. Falling through to Strategy 2/3...`);
                // Don't return — let it fall through to try PNCP planilha extraction
            }
            else {
                logger_1.logger.info(`[AI Populate] Extracted ${items.length} items (legacy mode)`);
        ...
        if (planilhaFiles.length === 0) {
            ...
            if (planilhaFiles.length > 0) {
                logger_1.logger.info(`[AI Populate] No planilha found — using ${planilhaFiles.length} edital/TR as source for item extraction`);
            }
        }
        // Debug: log all attachment purposes to diagnose classification issues
        if (attachments.length > 0) {
            logger_1.logger.info(`[AI Populate] Catalog has ${attachments.length} attachments. Purposes: ${JSON.stringify(attachments.map((a) => ({ t: a.titulo?.substring(0, 40), p: a.purpose, d: a.downloaded })))}`);
        }
        // ── Strategy 3: No catalog? Fetch attachments from PNCP API on the fly ──
        const pncpUrl = bidding.pncpLink || bidding.link || '';
        logger_1.logger.info(`[AI Populate] Strategy check: planilhaFiles=${planilhaFiles.length}, attachments=${attachments.length}, pncpUrl=${pncpUrl}, hasBiddingItems=${!!(biddingItems && biddingItems.trim().length >= 10)}`);
        if (planilhaFiles.length === 0 && pncpUrl) {
            logger_1.logger.info(`[AI Populate] No planilha in catalog (${attachments.length} total attachments). Fetching from PNCP: ${pncpUrl}`);
            ...
            if (pncpMatch) {
                ...
                try {
                    ...
                    const arquivosRes = await axios.get(arquivosUrl, { httpsAgent: agent2, timeout: 10000 });
                    const allArquivos = Array.isArray(arquivosRes.data) ? arquivosRes.data : [];
                    logger_1.logger.info(`[AI Populate] PNCP returned ${allArquivos.length} attachments`);
                    ...
                    if (planilhaFiles.length === 0) {
                        ...
                        if (planilhaFiles.length > 0) {
                            logger_1.logger.info(`[AI Populate] No planilha in PNCP fetch — using ${planilhaFiles.length} edital/TR instead`);
                        }
                    }
                    logger_1.logger.info(`[AI Populate] After PNCP fetch: ${planilhaFiles.length} candidates found`);
                }
                catch (fetchErr) {
                    logger_1.logger.warn(`[AI Populate] Failed to fetch PNCP attachments: ${fetchErr.message}`);
                }
            }
        }
        if (planilhaFiles.length === 0) {
        ...
        logger_1.logger.info(`[AI Populate] Found ${planilhaFiles.length} planilha candidates in PNCP catalog`);
        ...
        for (const pf of planilhaFiles.slice(0, 5)) { // Max 5 files
            try {
                logger_1.logger.info(`[AI Populate] Downloading: "${pf.titulo}" (${pf.purpose}) from ${pf.url}`);
                ...
                if (isPdf) {
                    ...
                    logger_1.logger.info(`[AI Populate] ✅ PDF: ${pf.titulo} (${(buffer.length / 1024).toFixed(0)} KB)`);
                }
                else {
                    logger_1.logger.info(`[AI Populate] ⚠️ Not a PDF: ${pf.titulo} — skipping`);
                }
            }
            catch (err) {
                logger_1.logger.warn(`[AI Populate] ⚠️ Failed to download ${pf.titulo}: ${err.message}`);
            }
        }
...
[{"itemNumber":"1.1","description":"Descrição completa do serviço incluindo grupo","unit":"M²","quantity":100,"multiplier":1,"multiplierLabel":"","referencePrice":45.67}]`;
        ...
        logger_1.logger.error('[AI Populate] Error:', error.message);
...
// ═══════════════════════════════════════════════════════════════════════
router.post('/ai-composition', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        logger_1.logger.info(`[AI Composition] Generating compositions for ${items.length} item(s), bidding: ${biddingProcessId}`);
        // Build items context
...
- NÃO retorne texto, markdown ou explicações — APENAS o JSON`;
        ...
        logger_1.logger.info(`[AI Composition] Response: ${responseText.length} chars in ${duration}ms`);
        let compositions;
        try {
            const parsed = JSON.parse(responseText);
        ...
        for (let idx = 0; idx < compositions.length && idx < items.length; idx++) {
            ...
            if (Math.abs(diff) >= 0.01 && targetPrice > 0) {
                ...
                logger_1.logger.info(`[AI Composition] Item ${idx + 1}: ajustado ${currentTotal.toFixed(2)} → ${adjustedTotal.toFixed(2)} (alvo: ${targetPrice.toFixed(2)}, diff original: ${diff.toFixed(2)})`);
            }
        }
        logger_1.logger.info(`[AI Composition] ✅ Generated ${compositions.length} compositions with ${compositions.reduce((s, c) => s + (c.lines?.length || 0), 0)} total lines in ${duration}ms`);
        res.json({
            compositions,
        ...
        logger_1.logger.error('[AI Composition] Error:', error.message);
...
router.post('/ai-letter', auth_1.authenticateToken, async (req, res) => {
    logger_1.logger.warn('[AI Letter] DEPRECATED endpoint called. Use /api/proposals/ai-letter-blocks instead.');
    res.status(410).json({
...
// ═══════════════════════════════════════════════════════════════════════
router.post('/ai-letter-blocks', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const t0 = Date.now();
        logger_1.logger.info(`[AI Letter Blocks] Generating ${requestedBlocks.length} block(s) for bidding ${biddingProcessId}`);
...
- Sem markdown, sem títulos, sem numeração.`;
        ...
        logger_1.logger.info(`[AI Letter Blocks] Completed in ${totalMs}ms — blocks: ${Object.keys(blocks).join(', ')} | timings: ${JSON.stringify(timings)}`);
        if (errors.length > 0) {
            logger_1.logger.warn(`[AI Letter Blocks] ${errors.length} block(s) failed:`, errors);
        }
        res.json({
            blocks,
            timings,
        ...
        logger_1.logger.error('[AI Letter Blocks] Error:', error.message);
        res.status(500).json({ error: 'AI block generation failed: ' + (error.message || 'Unknown') });
    }
});
// ═══════════════════════════════════════════════════════════════════════
router.post('/dossier/ai-match', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        logger_1.logger.info(`[Dossier AI Match] ${requirements.length} requirements × ${documents.length} docs for tenant ${req.user.tenantId}`);
        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey) {
            return res.status(500).json({ error: 'GEMINI_API_KEY not configured' });
        }
...
Responda somente com o JSON array, sem markdown, sem texto adicional:`;
        ...
        const responseText = result.text?.trim() || '';
        logger_1.logger.info(`[Dossier AI Match] Raw response (first 500 chars): ${responseText.substring(0, 500)}`);
        ...
        try {
            ...
            logger_1.logger.error('[Dossier AI Match] Failed to parse JSON:', responseText.substring(0, 500));
        ...
        for (const m of matchResults) {
            ...
            logger_1.logger.info(`[Dossier AI Match] ✅ R${reqIdx} → DOC[${docIdx}] "${documents[docIdx].docType}" | ${reason}`);
        }
        ...
        logger_1.logger.info(`[Dossier AI Match] Result: ${matchCount} matched, ${skipped} skipped, ${requirements.length - matchCount - skipped} unhandled`);
        res.json({ matches, matchCount, totalRequirements: requirements.length });
    }
    catch (error) {
        logger_1.logger.error('[Dossier AI Match] Error:', error?.message || error);
