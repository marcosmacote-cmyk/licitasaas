...
exports.generateImprovementInsights = generateImprovementInsights;
...
function generateImprovementInsights(periodDays = 30) {
    ...
    for (const fb of recentFeedback) {
        if (!moduleIssues[fb.moduleName])
            moduleIssues[fb.moduleName] = {};
        for (const issue of fb.issueTypes) {
            if (!moduleIssues[fb.moduleName][issue])
                moduleIssues[fb.moduleName][issue] = [];
    ...
    const modules = ['chat', 'petition', 'oracle', 'dossier', 'declaration', 'proposal'];
    for (const mod of modules) {
        const metrics = (0, operationalMetrics_1.generateModuleMetrics)(mod, periodDays);
    ...
    const goldenCandidates = (0, feedbackService_1.getFeedbackForGoldenConversion)();
    // Sort insights
    ...
    for (const ins of insights)
        moduleCount[ins.moduleName] = (moduleCount[ins.moduleName] || 0) + 1;
 ...
 */
function convertFeedbackToGoldenCases() {
    const candidates = (0, feedbackService_1.getFeedbackForGoldenConversion)();
    const converted = [];
...
// ── Helpers ──
function determineSeverity(moduleName, issueType, frequency) {
    ...
    if (criticalIssues.includes(issueType))
        return 'critical';
    if (highIssues.includes(issueType) && criticalModules.includes(moduleName))
        return 'critical';
    if (highIssues.includes(issueType) || frequency >= 5)
        return 'high';
    if (frequency >= 3)
        return 'medium';
    return 'low';
}
function determineProbableCause(issueType) {
    const causeMap = {
...
function mapIssueToComponent(issueType) {
    const componentMap = {
