...
class PncpSearchService {
     ...
     */
    static async searchGovbr(input, budgetMs = 12000) {
        const startTime = Date.now();
            ...
            if (useOfficialApi) {
                ...
                const fetchOfficialPage = async (pageNum, singleUf) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        ...
                        try {
                            ...
                            const resp = await axios_1.default.get(url, { httpsAgent: pncpKeepAliveAgent, timeout: axiosTimeout });
                ...
                const fetchWithRetry = async (url, retries = 2) => {
                    for (let attempt = 0; attempt <= retries; attempt++) {
                        ...
                        try {
                            const axiosTimeout = Math.min(8000, timeLeft());
                            const resp = await axios_1.default.get(url, { headers: { 'Accept': 'application/json' }, httpsAgent: pncpKeepAliveAgent, timeout: axiosTimeout });
    ...
    static async search(input) {
        ...
        let isStale = true;
        try {
            const syncState = await prisma_1.default.pncpSyncState.findUnique({ where: { id: 'singleton' } });
            isStale = !syncState ||
        ...
        const hasComplexFilters = !!(input.keywords ||
            input.orgao ||
            input.orgaosLista ||
            input.dataInicio ||
            input.dataFim);
        // UF, status, modalidade, esfera: OK local se banco estiver completo
        const needsRemote = localResponse.total === 0 || // zero local = vai remoto sempre
            isStale || // banco desatualizado = vai remoto sempre
            hasComplexFilters; // filtros complexos: local pode não cobrir
        if (!needsRemote) {
            logger_1.logger.info(`[PncpSearch] ✅ local-only: ${localResponse.total} resultados em ${localResponse.meta.elapsedMs}ms (stale=${isStale})`);
        ...
        if (budgetForRemote < 2000) {
            ...
            logger_1.logger.info(`[PncpSearch] ⚠️ Budget insuficiente para Gov.br (${budgetForRemote}ms), retornando local parcial`);
            return localResponse;
        }
        // 5. Remoto com budget
        logger_1.logger.info(`[PncpSearch] 🌐 Disparando Gov.br (local=${localResponse.total}, stale=${isStale}, complexFilters=${hasComplexFilters}, budget=${budgetForRemote}ms)`);
        try {
