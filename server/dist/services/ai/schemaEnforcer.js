...
function normalizeDateField(value) {
    ...
    if (/^\d{2}\/\d{2}\/\d{4}\s+às\s+\d{2}:\d{2}$/.test(v))
        return v;
 ...
 */
function normalizeCriterio(value) {
    if (!value || typeof value !== 'string')
        return '';
    const v = value.trim();
    if (!v)
 ...
 */
function extractMunicipioFromOrgao(orgao) {
    if (!orgao)
        return '';
...
//  MAIN ENFORCER
// ══════════════════════════════════════════════════════════════════
function enforceSchema(schema) {
    let corrections = 0;
    const details = [];
    const correct = (field, from, to) => {
        corrections++;
    ...
    if (schema.requirements) {
        ...
        for (const [category, items] of Object.entries(schema.requirements)) {
            ...
            for (const req of items) {
                ...
                if (!req.requirement_id || req.requirement_id.trim() === '') {
                    ...
                    while (allUsedIds.has(`${prefix}-${String(idCounter).padStart(2, '0')}`))
                        idCounter++;
                    const newId = `${prefix}-${String(idCounter).padStart(2, '0')}`;
                ...
                if (!req.source_ref || req.source_ref.trim() === '') {
                    if (Array.isArray(req.evidence_refs) && req.evidence_refs.length > 0 && Array.isArray(schema.evidence_registry)) {
                        const ev = schema.evidence_registry.find((e) => e.evidence_id === req.evidence_refs[0]);
                        if (ev) {
        ...
        for (const [category, items] of Object.entries(schema.requirements)) {
            ...
            const seen = new Map(); // normalized desc → index of first occurrence
            ...
            if (toRemove.size > 0) {
                const deduped = items.filter((_, idx) => !toRemove.has(idx));
                schema.requirements[category] = deduped;
        ...
        for (const [category, items] of Object.entries(schema.requirements)) {
            if (!Array.isArray(items))
                continue;
            for (const req of items) {
                if (!Array.isArray(req.sub_items) || req.sub_items.length <= 1)
                    continue;
                const normalizeForDedup = (text) => {
                    return (text || '')
                        .toLowerCase()
                ...
                req.sub_items = req.sub_items.filter((sub) => {
                ...
                if (req.sub_items.length < beforeLen) {
                    ...
                    correct(`${req.requirement_id}.sub_items`, `${removed} subitem(ns) duplicado(s)`, `removido(s) — de ${beforeLen} para ${req.sub_items.length}`);
                }
            }
        }
        ...
        for (const [category, items] of Object.entries(schema.requirements)) {
            ...
            if (items.length <= 1 && !hasSingleWrapperWithSubs)
                continue;
            const cleaned = items.filter((req) => {
                ...
                if (isGenericWrapper && (hasSubs || items.length > 1)) {
                    ...
                    if (hasSubs) {
                        const prefix = ID_PREFIX_BY_CATEGORY[category] || 'XX';
                        const existingIds = new Set(items.map((r) => r.requirement_id));
                        let counter = items.length + 1;
                        for (const sub of req.sub_items) {
                            if (!sub.requirement_id || existingIds.has(sub.requirement_id)) {
                                while (existingIds.has(`${prefix}-${String(counter).padStart(2, '0')}`))
                                    counter++;
                                sub.requirement_id = `${prefix}-${String(counter).padStart(2, '0')}`;
                            }
                            existingIds.add(sub.requirement_id);
                            // Ensure sub has required fields
                            if (!sub.obligation_type)
                                sub.obligation_type = req.obligation_type || 'obrigatoria_universal';
                            if (!sub.phase)
                                sub.phase = req.phase || PHASE_BY_CATEGORY[category] || 'habilitacao';
                            if (!sub.risk_if_missing)
                                sub.risk_if_missing = req.risk_if_missing || RISK_BY_CATEGORY[category] || 'inabilitacao';
                            if (!sub.applies_to)
                                sub.applies_to = 'licitante';
                            if (!sub.entry_type)
                                sub.entry_type = 'exigencia_principal';
                            // Inherit source_ref from parent if sub doesn't have one
                            if (!sub.source_ref && req.source_ref)
                                sub.source_ref = req.source_ref;
                            items.push(sub);
                            counter++;
                        }
    ...
    if (schema.requirements) {
        const qtoItems = schema.requirements.qualificacao_tecnica_operacional;
        const qtpItems = schema.requirements.qualificacao_tecnica_profissional;
        if (Array.isArray(qtoItems) && Array.isArray(qtpItems) && qtoItems.length > 2) {
            ...
            if (toMigrate.size > 0) {
                ...
                const migratedItems = [];
                const remainingQTO = qtoItems.filter((_, idx) => {
                ...
                const qtpText = qtpItems.map((r) => `${r.title || ''} ${r.description || ''}`.toLowerCase()).join('|');
                let added = 0;
                ...
                schema.requirements.qualificacao_tecnica_operacional = remainingQTO;
                // Renumber QTO
                remainingQTO.forEach((req, idx) => {
                    req.requirement_id = `QTO-${String(idx + 1).padStart(2, '0')}`;
                });
                // Renumber QTP
                qtpItems.forEach((req, idx) => {
    ...
    if (schema.requirements) {
        const rftItems = schema.requirements.regularidade_fiscal_trabalhista;
        const dcItems = schema.requirements.documentos_complementares;
        if (Array.isArray(rftItems) && Array.isArray(dcItems) && rftItems.length > 0 && dcItems.length > 0) {
            const rftNormalized = new Set(rftItems.map((r) => `${r.title || ''}`.toLowerCase()
                .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
                .replace(/[^a-z0-9 ]/g, '').trim().substring(0, 40)));
            const beforeLen = dcItems.length;
            const deduped = dcItems.filter((dc) => {
            ...
            if (deduped.length < beforeLen) {
                schema.requirements.documentos_complementares = deduped;
                // Renumber DC
                deduped.forEach((req, idx) => {
        ...
        if (Array.isArray(rftItems) && rftItems.length > 0) {
            ...
            if (toMigrate.length > 0) {
                ...
                const dcTitles = new Set(dcItems2.map((d) => `${d.title || ''}`.toLowerCase().substring(0, 30)));
                ...
                schema.requirements.regularidade_fiscal_trabalhista = rftCleaned;
                schema.requirements.documentos_complementares = dcItems2;
                // Renumber both
                rftCleaned.forEach((r, idx) => {
                    r.requirement_id = `RFT-${String(idx + 1).padStart(2, '0')}`;
                });
                dcItems2.forEach((r, idx) => {
    ...
    if (schema.requirements) {
        const pcItems = schema.requirements.proposta_comercial;
        if (Array.isArray(pcItems) && pcItems.length > 4) {
            ...
            const cleaned = pcItems.filter((req) => {
                const fullText = `${req.title || ''} ${req.description || ''}`;
            ...
            if (removedGeneric > 0) {
                schema.requirements.proposta_comercial = cleaned;
            ...
            const pcItemsAfterA = schema.requirements.proposta_comercial;
            const dcItems = schema.requirements.documentos_complementares || [];
            ...
            const toMigrateToDC = [];
            const pcAfterMigration = pcItemsAfterA.filter((req) => {
                const fullText = `${req.title || ''} ${req.description || ''}`;
            ...
            if (toMigrateToDC.length > 0) {
                ...
                schema.requirements.proposta_comercial = pcAfterMigration;
                schema.requirements.documentos_complementares = dcItems;
                correct('PC→DC', `${toMigrateToDC.length} item(ns) fiscal(is)`, `migrado(s): ${toMigrateToDC.map((r) => r.title).join(', ')}`);
            }
            // Renumber PC and DC after changes
            const finalPC = schema.requirements.proposta_comercial;
            if (Array.isArray(finalPC)) {
                finalPC.forEach((req, idx) => {
                    req.requirement_id = `PC-${String(idx + 1).padStart(2, '0')}`;
                });
            }
            const finalDC = schema.requirements.documentos_complementares;
            if (Array.isArray(finalDC)) {
                finalDC.forEach((req, idx) => {
                    req.requirement_id = `DC-${String(idx + 1).padStart(2, '0')}`;
                });
            }
        }
    ...
    if (schema.requirements) {
        ...
        if (Array.isArray(pcItems) && pcItems.length === 0) {
            ...
            const otherCatsPopulated = ['habilitacao_juridica', 'regularidade_fiscal_trabalhista',
                ...
                .some(cat => {
                const items = schema.requirements[cat];
            ...
            if (otherCatsPopulated) {
                ...
                schema.requirements.proposta_comercial = [
                    {
    ...
    if (pid) {
        ...
        if (pid.tipo_objeto && !VALID_TIPO_OBJETO.includes(pid.tipo_objeto)) {
            correct('tipo_objeto', pid.tipo_objeto, 'outro');
            pid.tipo_objeto = 'outro';
        }
        ...
        if (Array.isArray(rftItems) && rftItems.length >= 3) {
            ...
            if (cnpjIdx >= 0 && (ieIdx > cnpjIdx + 2 || imIdx > cnpjIdx + 2 || (ieIdx >= 0 && ieIdx !== cnpjIdx + 1) || (imIdx >= 0 && imIdx !== cnpjIdx + 2))) {
                ...
                for (let i = 0; i < rftItems.length; i++) {
                    ...
                    if (i === cnpjIdx) {
                        if (ieItem)
                            reordered.push(ieItem);
                        if (imItem)
                            reordered.push(imItem);
                    }
                }
                ...
                schema.requirements.regularidade_fiscal_trabalhista = reordered;
    ...
    if (schema.requirements) {
        ...
        if (Array.isArray(rftItems) && rftItems.length >= 2) {
            const allText = rftItems.map((r) => `${r.title || ''} ${r.description || ''}`.toLowerCase()).join(' ');
            // Derive source_ref from existing RFT items
            const rftSourceRef = rftItems.find((r) => r.source_ref && r.source_ref !== 'referência não localizada')?.source_ref || 'Edital, seção de Regularidade Fiscal';
            // Check for FGTS+INSS grouped in a single item and split them
            const fgtsInssGroupedIdx = rftItems.findIndex((r) => {
            ...
            const refreshedText = rftItems.map((r) => `${r.title || ''} ${r.description || ''}`.toLowerCase()).join(' ');
            const missingCNDs = [];
    ...
    if (schema.requirements) {
        const qefItems = schema.requirements.qualificacao_economico_financeira;
        if (Array.isArray(qefItems)) {
            const qefText = qefItems.map((r) => `${r.title || ''} ${r.description || ''}`.toLowerCase()).join(' ');
            ...
            if (qefItems.length <= 2) {
                const qefSourceRef = qefItems.find((r) => r.source_ref && r.source_ref !== 'referência não localizada')?.source_ref || 'Edital, seção de Qualificação Econômico-Financeira';
                const injected = [];
                if (!hasBalanco) {
                    qefItems.push({
                        requirement_id: '',
    ...
    if (schema.requirements) {
        const hj = schema.requirements.habilitacao_juridica || [];
        ...
        if (hj.length === 0 && otherCats) {
            const existingSourceRef = Object.values(schema.requirements)
                .flat()
                .find((r) => r.source_ref && r.source_ref !== 'referência não localizada')
                ?.source_ref || 'Edital, seção de habilitação';
            schema.requirements.habilitacao_juridica = [{
        ...
        if (schema.participation_conditions?.permite_consorcio === true) {
            const hjItems = schema.requirements.habilitacao_juridica || [];
            const hasConsorcioHJ = hjItems.some((r) => {
                const text = `${r.title || ''} ${r.description || ''}`.toLowerCase();
                return /cons[óo]rcio/.test(text);
            });
            if (!hasConsorcioHJ && hjItems.length > 0) {
                const hjSourceRef = hjItems.find((r) => r.source_ref && r.source_ref !== 'referência não localizada')?.source_ref || 'Edital, seção de habilitação jurídica';
                hjItems.push({
    ...
    if (ta) {
        ...
        const qtoCount = schema.requirements?.qualificacao_tecnica_operacional?.length || 0;
        const qtpCount = schema.requirements?.qualificacao_tecnica_profissional?.length || 0;
        ...
        if (ta.exige_cat === null && qtpCount > 0) {
            ...
            const qtpItems = schema.requirements?.qualificacao_tecnica_profissional || [];
            const hasCAT = qtpItems.some((r) => /CAT|certid[ãa]o\s+de\s+acervo/i.test(`${r.title || ''} ${r.description || ''}`));
            if (hasCAT) {
                ta.exige_cat = true;
