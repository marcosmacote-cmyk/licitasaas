...
import { getCategoriesBySeverity, DEFAULT_ENABLED_CATEGORIES, ALERT_TAXONOMY } from '../services/monitoring/alertTaxonomy';
import { normalizePortal, PLATFORM_DOMAINS } from '../lib/biddingHelpers';
import { isEncryptionConfigured, isEncrypted, decryptCredential } from '../lib/crypto';
...
let categoryKeywordMap: Map<string, string[]>;
let notifyAllSseClients: any;
let getFileBufferSafe: any;
let pncpMonitor: any;

export function injectChatMonitorDeps(deps: any) {
...
router.get('/taxonomy', authenticateToken, async (req: any, res) => {
    try {
...
router.patch('/pncp-link/:processId', authenticateToken, async (req: any, res) => {
    try {
...
router.get('/processes', authenticateToken, async (req: any, res) => {
    try {
...
router.post('/process-close/:processId', authenticateToken, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.get('/messages/:processId', authenticateToken, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.put('/log/:logId', authenticateToken, async (req: any, res) => {
    try {
...
router.put('/read-all/:processId', authenticateToken, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.put('/process-action/:processId', authenticateToken, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.get('/agents/sessions', authenticateToken, async (req: any, res) => {
    try {
        const tenantId = req.user.tenantId;
        const processes = await prisma.biddingProcess.findMany({
            where: {
...
router.get('/agents/status', authenticateToken, async (req: any, res) => {
    try {
        const tenantId = req.user.tenantId;
...
router.post('/internal/ingest', authenticateWorker, async (req: any, res) => {
    try {
...
router.get('/internal/notification-diag', authenticateWorker, async (req: any, res) => {
    try {
        const { NotificationService } = await import('../services/monitoring/notification.service');
...
router.post('/internal/reprocess-notifications', authenticateWorker, async (req: any, res) => {
    try {
        const { NotificationService } = await import('../services/monitoring/notification.service');
...
// ── Test Fetch BLL ──
router.get('/internal/test-bll-fetch', authenticateWorker, async (req: any, res) => {
    try {
...
router.patch('/internal/sessions/:processId/link', authenticateWorker, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.delete('/internal/sessions/:processId/logs', authenticateWorker, async (req: any, res) => {
    try {
        const { processId } = req.params;
...
router.post('/ingest', authenticateToken, async (req: any, res) => {
    try {
