...
exports.evaluateAnalysisQuality = evaluateAnalysisQuality;
exports.validateAnalysisCompleteness = validateAnalysisCompleteness;
...
const checks = [
    ...
    { code: 'N02', dimension: 'normalization', weight: 3, check: (s) => {
            ...
            const nonEmpty = categories.filter(k => s.requirements[k]?.length > 0);
...
// ── Executor ──
function evaluateAnalysisQuality(schema, ruleFindings, analysisId) {
    const issues = [];
    ...
    const rawOverall = Math.round(categoryScores.extraction * 0.25 +
    ...
    logger_1.logger.info(`[QualityEval] ${summary}`);
    return {
