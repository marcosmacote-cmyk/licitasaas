...
INVERSÃO DE FASES (Art. 17, §1º):
  ...
  ⚠️ ATENÇÃO MÁXIMA: A sequência padrão (propostas ANTES de habilitação) NÃO é inversão de fases. Inversão é quando a habilitação vem PRIMEIRO. Este conceito é frequentemente confundido — NÃO erre.
  ● PREGÃO: segue a ordem padrão da lei (propostas → habilitação). Isto NÃO constitui "inversão de fases".
  ● CONCORRÊNCIA: também segue a ordem padrão (propostas → habilitação) SEM que isso constitua inversão. Só haverá inversão se o edital EXPLICITAMENTE determinar que a habilitação ocorrerá ANTES das propostas.
  ● COMO VERIFICAR: Para determinar se o edital adota inversão de fases, procure cláusula EXPRESSA dizendo "a habilitação antecederá o julgamento das propostas" ou "adota-se a inversão de fases nos termos do Art. 17, §1º". Se o edital diz que a habilitação será exigida "após o julgamento" ou "somente do primeiro classificado", isto é a ORDEM PADRÃO, não inversão.
...
═══ ARMADILHAS CONCEITUAIS (☢️ CONCEITOS FREQUENTEMENTE CONFUNDIDOS) ═══

Antes de responder sobre QUALQUER conceito jurídico abaixo, CONSULTE OBRIGATORIAMENTE a base de conhecimento acima. Se sua resposta contradizer a base, a BASE PREVALECE:

☢️ INVERSÃO DE FASES: Na Lei 14.133/2021, a ordem padrão já é propostas → habilitação. Inversão de fases = habilitação ANTES das propostas (Art. 17, §1º). Se o edital conduz propostas/lances primeiro e depois habilitação, NÃO há inversão — é a ordem padrão. NUNCA diga que "segue a inversão de fases" só porque habilitação é após proposta.
☢️ CERTIDÃO DE QUITAÇÃO vs. REGULARIDADE: Súmula TCU 283 — só se exige regularidade, nunca quitação.
☢️ SOMATÓRIO DE ATESTADOS: regra geral = permitido. Vedação = excepcionalíssima.
☢️ CAPITAL SOCIAL vs. PATRIMÔNIO LÍQUIDO: NÃO podem ser exigidos cumulativamente (Art. 69, §2º).
☢️ GARANTIA DE PROPOSTA vs. GARANTIA CONTRATUAL: São instâncias distintas com limites diferentes (1% vs. 5%).
...
exports.CHAT_USER_INSTRUCTION = `Com base na análise do edital fornecida no contexto e na BASE DE CONHECIMENTO JURÍDICO do sistema, responda à pergunta do usuário de forma PRECISA e OPERACIONAL.

Pergunta: {userQuestion}

REGRAS DESTA RESPOSTA:
- Use definições EXATAS da base de conhecimento jurídico para conceitos legais
- ⚠️ ANTES de usar qualquer conceito jurídico (inversão de fases, inexequibilidade, empate ficto, etc.), RELEIA a definição na BASE DE CONHECIMENTO e verifique se sua resposta está COERENTE com ela
...
- Se a pergunta envolver ARMADILHA CONCEITUAL (☢️), valide sua resposta contra a seção de armadilhas antes de responder`;
