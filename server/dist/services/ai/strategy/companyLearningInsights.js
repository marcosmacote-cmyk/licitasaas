...
async function recordMatchHistory(companyId, processId, scores, recommendation) {
    try {
        ...
        if (!config.aiHistoryStore)
            config.aiHistoryStore = {};
        if (!config.aiHistoryStore[companyId])
            config.aiHistoryStore[companyId] = [];
